...
export default function AboutSection({ about, update, preview, onToast, onGoSection }) {
  ...
  /** 触发画廊多选图片 */
  const pickGallery = () => galleryInput.current?.click()
  ...
  const onGalleryFiles = async (e) => {
    ...
    update((a) => ({
      gallery: [...a.gallery, ...imgs.map((src) => ({ id: Math.random().toString(36).slice(2, 9), src }))]
    }))
    setGalleryBusy(false)
    onToast(`已添加 ${imgs.length} 张图片`)
    e.target.value = ''
  }
      ...
      <div className="hero">
        ...
        <div>
          ...
          {!preview && (
            <div className="notes-row">
              {about.notes.map((n) => (
                <span key={n.id} className="note-chip">
                  {n.text}
                  <button className="n-del" onClick={() => removeNote(n.id)}>✕</button>
                </span>
              ))}
        ...
        <div className="avatar-wrap">
          <div className="avatar-box card-parent" onClick={pickAvatar}>
            ...
            <div className="avatar-tip">点击替换头像</div>
          </div>
          ...
          {!preview && !about.nowBadge?.visible && (
            <button className="btn btn-subtle now-card" style={{ left: -26, bottom: 30, position: 'absolute' }}
      ...
      <div className="gallery-area">
        ...
        {!preview && (
          <div className="gallery-ctrl">
            <button className="btn btn-subtle" onClick={pickGallery}>+ 增加图片</button>
            <button className="link-btn" onClick={clearGallery}>清空图片</button>
            <span className="gallery-count">{about.gallery.length} 张</span>
          </div>
        )}
      </div>
